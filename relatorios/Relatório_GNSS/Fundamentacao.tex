...
A impedância característica de uma trilha é dada pela Equação (\ref{eq:impedancia_trilha}):
...
\begin{itemize}
    \item $Z_0$: impedância característica da trilha ($\Omega$)
    \item $\epsilon_r$: permitividade relativa do material da PCB
...
\begin{itemize}
    \item $\epsilon_r = 4.5$: varia entre 3,5 e 5,5; assumimos a média.
    \item $h = 1.5\,\un{mm}$: altura do dielétrico para placas de duas camadas (fornecidas pelo fabricante).
    Se a placa fosse de quatro camadas, poderia ser $h = 0.508\,\un{mm}$.
    \item $w = 0.254\,\un{mm}$: largura de trilha padrão; pode ser ajustada no roteador (EasyEDA).
    \item $t = 0.35\, \mu \un{ m}$: espessura do condutor (valor padrão informado).
\end{itemize}

\noindent Isso resulta em $Z_0 = 129,78 \, \Omega$.
