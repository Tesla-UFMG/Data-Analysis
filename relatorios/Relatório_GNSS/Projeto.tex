...
Além disso, o Teseo opera apenas na faixa de frequência de 1575 MHz (1.5 GHz - 1.6 GHz). Utilizar antenas que recebem sinais fora dessa faixa,
proveninentes de constelações de GNSS L2 mais modernas que operam em 1.2 GHz, não irão funcionar com esse circuito.
...
\begin{itemize}
    ...
    \item A largura das trilhas de RF é 0,762 mm (30 mils), com 0,508 mm (20 mils) de separação entre a trilha e os planos de GND adjacentes. Isso resulta em
    $Z_0 = 73 \, \Omega$ para uma PCB de duas camadas (nosso caso) e
    $Z_0 = 53,93 \, \Omega$ para quatro camadas com 0,508 mm entre as camadas externas e
...
Uma outra possibilidade é usar uma máquina de estados dentro do \verb|while(1)|,
removendo a necessidade do FreeRTOS. Pode haver dois estados:

\begin{enumerate}
    \item \verb|STATE_RECEIVE_UART|: chamado quando a interrupção da UART (com DMA) indicar que novos dados cheagaram. 
    Esse estado processa as strings NMEA e atualiza a struct global.
    \item \verb|STATE_SEND_UART|: envia os dados da struct global via CAN (ou via outro módulo UART conectado ao computador, para testes).
    Esse estado é chamado periodicamente
    usando um TIMER configurado para gerar uma interrupção a cada 200 ms (5 Hz).
\end{enumerate}
...
Caso seja necessário usar algum módulo TIMER para executar uma função a um intervalo 
de tempo constante, use a expressão em (\ref{eq:timer}) para calcular o período do TIMER:

\begin{equation}\label{eq:timer}
    T = \frac{(ARR + 1)(PSC + 1)}{f_{clock}}
\end{equation}

\noindent onde:

\begin{itemize}
    \item ARR: valor do auto-reload register (ARR) do TIMER
    \item PSC: valor do prescaler (PSC) do TIMER
    \item $f_{clock}:$ frequência de clock do TIMER, veja na aba Clock Configuration do IOC
\end{itemize}
...
\begin{enumerate}
    \item Aumentar baud rate para 115200: PSTMSETPAR,3102,0xA
    \item Aumentar taxa de amostragem para 5 Hz: PSTMSETPAR,1303,0.2
\end{enumerate}

Os comandos acima devem ser enviados via UART para o Teseo. Eles podem ser enviados 
usando o \verb|HAL\_UART\_Transmit()| do STM na seguinte sequência:
...
Note que ao mudar o baud rate, é necessário reconfigurar o periférico UART do STM
para o novo baud rate.

As mensagens têm a seguinte estrutura:

\begin{verbatim}
    $ {command},{param},...,{param} * {checksum} \r\n
\end{verbatim}

\noindent onde:

\begin{itemize}
    \item \verb|$|: início da mensagem
    \item \verb|{command}|: comando a ser enviado, veja no manual do software do Teseo
    \item \verb|{params}|: parâmetros do comando, veja no manual do software do Teseo
    \item \verb|*|: separador entre os parâmetros e o checksum
    \item \verb|{checksum}|: soma XOR de todos os bytes entre o caractere \$ e o caractere * (não incluindo esses caracteres)
    \item \verb|\r\n|: carriage return e nova linha
\end{itemize}

É possível usar um código simples em python para calcular o checksum de qualquer comando:
...
Se for necessário voltar para a configuração padrão do Teseo, envie o comando
\verb|$PSTMRESTOREPAR| com o devido checksum e estrutura acima.


\section{Posicionamento no Carro (completar após teste)}
